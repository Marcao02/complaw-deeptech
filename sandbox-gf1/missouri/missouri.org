...
at this rate we will complete the full sentence on [2018-02-20 Tue]

* modeling de morgan's cat

∀cat.  switchedOn(cat) → ∀rodent ∈ mice ∪ rats →  catch(cat,rodent);
∀cat. switchedOff(cat) → ∀rodent ∈ mice U rats → ¬catch(cat,rodent);
