...
// extract a schema from a commonform file
...
fs.readFile(filename, 'utf8', function(err, data) {
  ...
  if      (ops.directions) { console.log(JSON.stringify(parsed.directions)) }
  else if (ops.content)    { console.log(JSON.stringify(parsed.form.content)) }
  else if (ops.schema)     { console.log(JSON.stringify(d2s(parsed.directions))) }
