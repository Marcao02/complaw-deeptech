...
\newcommand{\deinst}{\termz{einst}{external-event instance}}
\newcommand{\penames}{\termz{penames}{\text{party-events}}}
\newcommand{\oenames}{\termz{oenames}{\text{oracle-events}}}
\newcommand{\denames}{\termz{denames}{\text{internal-events}}}
\newcommand{\dename}{\termz{denames}{\text{internal-event}}}
...
\item \anchorfor{\oenames}{oenames}, which are events created by \oracle-\actors which provide information from the environment, and
\item \anchorfor{\denames}{denames}, which are transitions mandated by the contract.
\end{PPI}
An \anchorfor{event instance}{einst} is a tuple $\tup{e,a,\vts,\sigma}$ where $e$ is an \ename, $a$ is an \textit{optional} \actor, $\vts$ is a \timestamp, and $\sigma \in \eppartsorts{e}$. The \actor component is omitted iff $e$ is a \dename.
...
A \drule  $r$ governing the applicability of a \dename $e$ has an additional {\it trigger function} $\anchorfor{\deadfn{r}}{deadfn}$ of type
...
where the \timestamp argument is the \timestamp of the previous \einst. Thus, all the \dename parameters are completely determined by the rule and the current state. This makes sense because an \deinst is created by the contract itself, which is deterministic.
...
\item If $E_i$ is a \deinst, then it must be \compatiblewith the unique\footnote{by the \udc} \drule $r$ of $s$ that is enabled in $G_i$ such that $\deadfn{r}(\vts, \pi) = \vts'$. Moreover, $\sigma$ must be $\psetter{r}(t,\pi)$.
...
\item If $E_i$ is a \deinst, then it must be \compatiblewith the unique\footnote{by the \udc} \drule $r$ of $s$ that is enabled in $G_i$ such that $\deadfn{r}(\vts, \pi) = \vts'$. Moreover, $\sigma$ must be $\psetter{r}(\vts,\pi)$.
