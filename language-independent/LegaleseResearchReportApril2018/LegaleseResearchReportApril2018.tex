...
\newcommand{\party}{\termz{parties}{party}}
\newcommand{\parties}{\term{parties}}
\newcommand{\oracle}{\termz{oracles}{oracle}}
\newcommand{\oracles}{\term{oracles}}

\newcommand{\contractOracle}{\termz{contractOracle}{Contract}}
...
\newcommand{\eeinst}{\termz{einst}{external-event instance}}
\newcommand{\ieinst}{\termz{einst}{internal-event instance}}
\newcommand{\penames}{\termz{penames}{\text{party-events}}}
\newcommand{\oenames}{\termz{oenames}{\text{oracle-events}}}
\newcommand{\eenames}{\termz{eenames}{\text{external-events}}}
\newcommand{\eename}{\termz{eenames}{\text{external-event}}}
\newcommand{\ienames}{\termz{denames}{\text{internal-events}}}
\newcommand{\iename}{\termz{denames}{\text{internal-event}}}
...
\newcommand{\psetter}[1]{{\sf \hyperref[psetter]{psetter}}_{#1}} %for \drules only
\newcommand{\actorsAllowed}[1]{{\sf \hyperref[actors]{actorsAllowed}}_{#1}} %for external event rules only
...
\newcommand{\deadfn}[1]{\EM{\textsf{\hyperref[deadfn]{trigger}}_{#1}}} % ep for 'event parameters'
...
\newcommand{\arules}{\termz{arules}{external-event rules}}
\newcommand{\arule}{\termz{arules}{external-event rule}}
\newcommand{\drules}{\termz{drules}{internal-event rules}}
\newcommand{\drule}{\termz{drules}{internal-event rule}}
...
\newcommand{\udc}{\termz{udc}{unambiguous external\-transition condition}}
...
\begin{PPI}
\item \anchorfor{\eenames}{eenames}, which is the disjoint union of:
    \begin{PPI}    
	\item \anchorfor{\penames}{penames}, which are actions done \party-\actors.
	\item \anchorfor{\oenames}{oenames}, which are events created by \oracle-\actors which provide information from the environment.
    \end{PPI}	
\item \anchorfor{\ienames}{ienames}, which are transitions mandated by the contract.
\end{PPI}
An \anchorfor{event instance}{einst} is a tuple $\tup{e,a,\vts,\sigma}$ where $e$ is an \ename, $a$ is an \textit{optional} \actor, $\vts$ is a \timestamp, and $\sigma \in \eppartsorts{e}$. The \actor component is omitted iff $e$ is a \iename.
...
\item $\anchorfor{\breached{A}}{breached}$ for each nonempty subset $A$ of $\actor$.\footnote{These are breaches and oracle errors analogous to undifferentiated, unhandled exceptions in normal programming languages. Some well-drafted computational contracts might avoid using them completely.}
\end{PPI}
...
A \drule  $r$ governing the applicability of a \iename $e$ has an additional {\it trigger function} $\anchorfor{\deadfn{r}}{deadfn}$ of type
...
where the \timestamp argument is the \timestamp of the previous \einst. Thus, all the \iename parameters are completely determined by the rule and the current state. This makes sense because an \ieinst is created by the contract itself, which is deterministic.

Since \drules cause an event to occur automatically when the rule activates, we would need to either specify what happens when two such rules activate at the same time, or else ensure that can't happen. We take the latter approach.\footnote{Because we see no natural way to pick one over the other. Note that the \erules are not ordered.} For now, we adopt a constraint that is stronger than necessary but especially simple:\footnote{A closer-to-minimal constraint is: when the \enabledguards of two \drules are simultaneously \True, their trigger functions cannot yield the same \timestamp (and then the earlier of the two \timestamps is used). We have not yet experienced any desire for the extra leniency, but in case we do, it would be easy to allow it.} \\
\noindent \anchorfor{\udc}{udc}:  if a \shandler has multiple \drules, then their \enabledguards must be disjoint relations.

%\centerline{\begin{fmpage}{5.5in}
\begin{bnote}[redundancy of $\reguard{r}$ for \actor rules] \label{guardredundancy}
With respect to the execution semantics (Section \ref{execution}), it is easy to eliminate $\reguard{r}$ for party and oracle \erules $r$ (though not for \drules), by conjoining $\reguard{r}$ to $\eppartconstr{r}$. We will do this for some formal verification purposes where the simplification of the model outweighs the cost (in development time) of interpreting errors and \traces. From the practical software engineering perspective, on the other hand, we have found it is very natural to split the constraint on when an \actor \erule can apply into
\begin{LPPI}
...
As a very basic liveness condition, we want that an \lsm can't get stuck in a \state where no \actor \erules will ever be applicable again, and where there is no \drule that will trigger. We call this the \anchorfor{\nsc}{nsc}.


Each \arule $r$ governing the applicability of a \actor \ename $e$ has an additional {\it parameter constraint relation} \anchorfor{$\eppartconstr{r}$}{epconstr} on
...
Note that a {\it parameter setter} is a special case of a {\it parameter constraint relation}. Because that special case is used fairly frequently, in L4 we allow \arules to use the {\it parameter setter} syntax of \drules instead of their own {\it parameter constraint relation} syntax.
\begin{center}
...
\item If $E_i$ is a \ieinst, then it must be \compatiblewith the unique\footnote{by the \udc} \drule $r$ of $s$ that is enabled in $G_i$ such that $\deadfn{r}(\vts, \pi) = \vts'$. Moreover, $\sigma$ must be $\psetter{r}(t,\pi)$.
...
\item For each \actor \erule $r$ that \governs an \ename $e$:
	\begin{LPPI}
	\item $\eppartconstr{r} : \timestamp^2 \times \svsorts \times \eppartsorts{e} \rarr \Bool$
	\end{LPPI}
\item For each \drule $r$ that \governs an \ename $e$.
	 \begin{LPPI}
...
\item For each $e \in \ename$:
\begin{PPI}
\item a subset of \actor; the \actors allowed to initiate an \einst of type $e$.
\item a destination \sname $\edest{e}$.
\item types for its parameters $\eppartsorts{e} \in \DT^*$.
\item a \stransimpl, $\statetransform{e}$, which is a set of $\estatements{e}$ that satisfies the \uuc. The functions used on the right side of assignment statements are from $\cFstar$.
\end{PPI}
...
\begin{PPI}
	...
	\item A subset $\actorsAllowed{r}$ of \actors.
\end{PPI}
\item For each \drule $r$ that \governs an \ename $e$.
	 \begin{PPI}
...
\item If $E_i$ is a \ieinst, then it must be \compatiblewith the unique\footnote{by the \udc} \drule $r$ of $s$ that is enabled in $G_i$ such that $\deadfn{r}(\vts, \pi) = \vts'$. Moreover, $\sigma$ must be $\psetter{r}(\vts,\pi)$.
...
First, and unrelated to the following \timestamp-related transformations (but with the same goal of simplifying symbolic execution), we eliminate $\reguard{r}$ for each \actor \erule $r$, as described in Note \ref{guardredundancy}. The remainder of the transformations consists of: 
\begin{LPPI}
\item Converting the contract state's current \timestamp to a regular \statevar.
\item Converting \drules to \arules, by introducing a new \oracle \actor \contractOracle.
\end{LPPI}
In detail:
\begin{LPPI}
\item Introduce a new \statevar $\tlast$ of sort \timestamp with initial value $0$. 
%In the technical terminology: the new versions $\svsorts'$, $\initvals'$, $\statevar'$ are the same as the old $\svsorts$, $\initvals$, $\statevar$, but with the additions: $\svsorts'(\tlast) = \timestamp$ and $\initvals'(\tlast) = 0$ and $\statevar' = \statevar + \tlast$.
\item Add a parameter $\tnext$ of sort \timestamp to every \ename.
%\footnote{i.e. $\eppartsorts{e}(\tnext) = \timestamp$ for every \ename $e$.}
\item Add $\assign{\tlast}{\tnext}$ to $\statetransform{e}$ for each $e$.
\item Add $>_{ts} {:} \timestamp \times \timestamp \rarr \Bool$ to $\cF$.
\item Each \arule $r$ gets converted to an \arule $r'$ that is identical save for::
\[
 \eppartconstr{r'}((\vq, \tlast), (\vx, \tnext)) := \\ 
 \eppartconstr{r}(\tlast, \tnext, \vq, \vx)\, \wedge\, \tnext >_{ts} \tlast 
\]
\item Each \drule $r$ gets converted to an \arule $r'$ that is identical save for having 
\begin{PPI}
	\item $\actorsAllowed{r} = \{ \contractOracle \}$
	...
	\item $\psetterpart{r'}{\tnext}(\vq) := \deadfn{r}(\tnext, \vq)$.
\end{PPI}	
\end{LPPI}

{\bf Issue:} We have to be careful to emulate the semantics of \drules rules, especially the \udc and the \nsc. \udc should get renamed something like ``unambiguous external-transition condition``. \\
...
\item \anchorfor{\eparams{e}}{eparams} for the tuple of $e$'s parameters (i.e. $\eppart{1}{e}, \ldots, \eppart{k}{e}$ if $e$ has $k$ parameters).
...
\todomin{Roles most ignored atm! Consider eliminating them from \timelesslsmclass{\cF}.}
...
&	\stackone{\eptosmt; \svtosmt; P' \vdash \yields{\ehandlerof{e}}{\rv} }
			{\sigma = \eparams{e} \mapsto \psetterpart{e}{1}(\pi),\ldots,\psetterpart{e}{k}(\pi)}
}
\]
...
\infer{
	\eptosmt; \svtosmt ; P \vdash \yields{   \{ \assign{q_{i_1}}{t_{i_1}}, \ldots, \assign{q_{i_m}}{t_{i_m}}  \}   }{\rv}
	 	...
	 	{\svtosmt' ; P \vdash \yields{\edest{e}}{\rv} }
