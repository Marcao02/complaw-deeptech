...
\newcommand{\dename}{\termz{denames}{\text{deadline-event}}}
...
where the \timestamp argument is the \timestamp of the previous \einst. Thus, all the deadline \ename parameters are completely determined by the rule and the current state. This makes sense because a deadline \einst is created by the contract itself, which is deterministic.
...
As a very basic liveness condition, we want that an \lsm can't get stuck in a \state where no \actor \erules will ever be applicable again, and where there is no deadline \erule that will trigger. We call this the \anchorfor{\nsc}{nsc}.


Each \actor event rule $r$ governing the applicability of a \actor \ename $e$ has an additional {\it parameter constraint relation} \anchorfor{$\eppartconstr{r}$}{epconstr} on
...
First, and unrelated to the following \timestamp-related transformations (but with the same goal of simplifying symbolic execution), we eliminate $\reguard{r}$ for each \actor \erule $r$, as described in Note \ref{guardredundancy}. Next:
\begin{LPPI}
...
\item For each party/oracle \erule $r$:
\[
 \eppartconstr{r}'(\vq, \vx) := \eppartconstr{r}(\tlast, \tnext, \vq, \vx) \wedge \tnext >_{ts} \tlast
\]
