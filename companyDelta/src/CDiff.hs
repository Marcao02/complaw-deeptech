...
instance RDiff CompanyState where
  rdiff
    ...
    let forest = catMaybes $ pruneNoops <$>
          [ rdiff holders1 holders2,
            rdiff se1 se2,
            rdiff co1 co2,
            rdiff holdings1 holdings2 ]
    in mytrace ("rdiff CompanyState") (
      if length forest > 0
      then Node (Update s1 s2 "companystate change")
      else Node (Noop "no change to companystate")) forest

instance RDiff Company where
  rdiff
    s1@(Company n1 j1 idtype1 idnum1)
    s2@(Company n2 j2 idtype2 idnum2) =
    let forest = catMaybes $ pruneNoops <$> [ rdiff n1 n2
                                            , rdiff j1 j2
                                            , rdiff idtype1 idtype2
                                            , rdiff idnum1 idnum2 ]
    in (if length forest > 0
        then Node (Update s1 s2 "change company")
        else Node (Noop "no change to company")) forest
...
instance RDiff Holder where
  rdiff
    ...
    let forest = catMaybes $ pruneNoops <$> [ rdiff fullname1 fullname2
                                            , rdiff idtype1   idtype2
                                            , rdiff idnum1    idnum2
    ...
    mytrace ("rdiff Holder")
    (if length forest > 0
     then Node (Replace s1 s2 "change holder")
     else Node (Noop "no changes to holder")) forest


instance RDiff [Security] where
  rdiff s1s s2s =
    mytrace ("rdiff [Security]")
    (
...
instance RDiff Security where
  rdiff s1@(Security name1 measure1)
    ...
    let forest = catMaybes $ pruneNoops <$> [ rdiff name1 name2 ]
    in mytrace ("rdiff Security:" ++ name1) (
      if length forest > 0
      then Node (Replace s1 s2 "change security")
      else Node (Noop "no change to security") ) forest
...
instance RDiff [Holding] where
  rdiff s1s s2s =
    ...
    in
      ...
      (if length forest > 0
       then Node (Update s1s s2s "updates to holdings")
       else Node (Noop "no update to holdings")) forest
  ...
  rdiff s1@(Holding holder1 holds1)
    ...
    in
      ...
      (if length forest > 0
       ...
       else Node (Noop (holder1 ++ "'s holder/securities did not change"))) forest
...
instance RDiff [HeldSecurity] where
  rdiff hs1s hs2s =
    let hsBefore = ignoreDescription $ myconcat $ Just hs1s
        ...
        forest   = catMaybes $ pruneNoops <$> [ rdiff hsBefore hsAfter ]
    in 
...
instance RDiff HeldSecurity where
  rdiff s1@(HeldSecurity securityName1 units1 money1 description1)
    ...
    let forest = catMaybes $ pruneNoops <$> [ rdiff units1 units2
                                            , rdiff money1 money2
                                            , rdiff description1 description2 ]
    in
      mytrace ("rdiff HeldSecurity: " ++ securityName1 ++ "=" ++ securityName2)
      ( if length forest > 0
        then Node (Replace s1 s2 "changing held securities") forest
        else Node (Noop "no change to held securities") [] )
