...
We say that \atransition $\eva$ and an \Event $E = \tup{a,r,t}$ are \introx{compatible}{compatible} iff they have the same \Action $a$ and the same \Role $r$. This definition will be modified in Section \ref{globalvars} when we add \Event parameters.
\medskip
...
Each \transition $\eva$ is also associated with a \introxx{TGuard}{edge guard}{TGuards}{edge guards} $\tguard{\eva}{\cdot}$ relation. For \FSContracts, it is just a relation on \TimeStamps,
and \atransition $\eva$ is \introx{enabled}{enabled} upon entering a \GlobalState with \TimeStamp $t$ iff $\tguard{\eva}{t}$ is true.\footnote{Currently, LSM examples are written assuming the \TGuards of a \State s's \transitions get evaluated only once upon entering the \State. It would also be reasonable to guess that they get evaluated once per \TimeUnit while the \Contract is in that state. This is not ideal.}  

\medskip

Each \transition $\eva$ is also associated with a \introx{DeadlineFn}{deadline function} $\deadline{\eva}{\cdot}$, which yields a \introxx{Deadline}{deadline}{Deadlines}{deadlines}. $\deadline{\eva}{t}$ is either a \TimeStamp after $t$, or the special element \introx{nodeadline}{$\infty$}. The \Deadline for \atransition is when:
...
\begin{PPI}
    ...
    \item Otherwise, $E$ must be $\breach{\role{\eva}}{\deadline{\eva}{t} + 1}$. %Otherwise, If $E$ is not \compatible with $\eva$ then $\role{\eva}$ solely breaches the contract. We specify that $E$ must be 
%    \item[(c)] If $E$ is \compatible with $\eva$ and $E$ happens after $\eva$'s deadline, then $\role{\eva}$ solely breaches the contract. 
\end{PPI}
...
%\pagebreak
%%\item If $t'$ is within $\eva$'s deadline, there are two subcases: 
%\begin{LPPI} 
%\item $A$'s \Role and \Action are not the same as $\eva$'s. This means $A$'s \Role performed some action while $\eva$'s role had an immediate obligation and still had time to fulfill it.
%
%\item If the next \GlobalState $G'$ is consistent with {\bf Case 2} below,\footnote{i.e. it would be a valid next \GlobalState if $\eva$ was removed from the contract.} that's fine. {\bf Importantly, \Events are really {\it action decisions}; they do not consume time. So, if Alice has an immediate obligation, it is not possible for Bob to ``starve'' Alice by repeatedly doing permitted \Events.} 
...
%
%Otherwise, the next state must be $\trg{\eva}$.\footnote{i.e. if $t' \leq \deadline{\eva}{t}$ then $s' = \trg{\eva}$.} This means $A$ fulfilled the obligation created by $\eva$. \\
%
%Otherwise ($t'$ is past $\eva$'s deadline), the next state {\it must} be $\eva's$ \Role's \breach state, and moreover we specify that $t'$ is exactly one \TimeUnit past the deadline.\footnote{So $G' = \tup{\breached{\{\role{\eva}\}}, \deadline{\eva}{t} + 1}$.} Thus, when a \breach occurs because of a missed deadline, the final  \TimeStamp is maximally early.\\
%
%{\bf Case 2}: There is no \enabled \mustntran. Let $T$ be the \enabled \mayntrans of $s$.\footnote{i.e. the set of $\eva$ such that $\tguard{\eva}{t}$ is true.} Compute the deadline for each \transition in $T$, and discard the \transitions whose deadline has passed by the time $A$ happens.\footnote{i.e. discard $\eva$ if $\deadline{\eva}{t} > t'$.} Then $A$ must be \compatible with one of the \transitions in that filtered set.
