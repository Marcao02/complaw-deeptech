...
\introxxoutside{AParamsDomain}{action-parameters domain}{AParamsDomains}{action-parameters domains}
\introxoutside{gvarsassign}{global vars assignment}.
\begin{LPPI}
\item A fixed finite set of typed \introxxinside{global vars}. The \GVars are ordered, so we may describe their collective types as a single tuple $\introxinside{GVarTypes} \in \TT^* $\\
 Add to the definition of \GlobalState an assignment of values to the \GVars. We'll call such an assignment a \introxinside{global vars assignment}.
...
\end{LPPI}
%\introxarg{itsAParamsDomain}{paramtypes}$.
...
\begin{LPPI}
\item Each \Action $a$ additionally has a \gvTransform, denoted \itsTransform{a}, which is a function from $\GVarTypes \times \itsAParamsDomain{a} \times \TimeStamps$ to \GVarTypes. 
\item The definition of the \TGuard of a \depTrans{a} is generalized: it may now depend on the values of the \GVars; i.e. it is now a relation on $\TimeStamps \times \text{\GVarTypes}$.
\end{LPPI}
