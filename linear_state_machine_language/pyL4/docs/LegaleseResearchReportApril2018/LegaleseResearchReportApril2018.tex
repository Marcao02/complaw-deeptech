...
\newcommand{\svsorts}{\termz{svsorts}{\EM{\textsf{\hyperref[svsorts]{sorts}}_{\textsf{state}}}}}
...
\newcommand{\epsorts}[1]{\textsf{\hyperref[epsorts]{sorts}}_{#1}}
...
\noindent
\todo{Closure of $\cF$ under composition}\\
\todo{Kill $\epsorts{e}$ in favour of the one-sort-per-name requirement}\\
\todo{Maybe kill $\svsorts$ for the same reason}\\
...
\mybox{\begin{bnote}
Every typed symbol is used with a unique sort (i.e. element of $\DT$), regardless of scope. 
\end{bnote}}
...
Fix a finite set of symbols \firstuse{event}{ename}, and for each such $e$ a parameter type assignment $\epsorts{e} \in \DT^*$. Furthermore, partition \ename into three kinds of events:
\begin{PPI}
...
\end{PPI}
An \firstuse{event instance}{einst} is a tuple $\tup{e,a,t,\sigma}$ where $e$ is an \ename, $a$ is an \actor, $t$ is a \timestamp, and $\sigma \in \epsorts{e}$. The \actor for a deadline \ename is always \codeactor.
...
\[\timestamp^2 \times \svsorts \times \epsorts{e} \rarr \svsorts\]
\end{PPI}
...
\[ \timestamp \times \svsorts \rarr \epsorts{e}  \]
...
\[ \timestamp^2 \times \svsorts \times \epsorts{e}\]
where the two \timestamp arguments are the \timestamps of the previous and next \einsts.
...
\begin{LPPI}
\item $\statetransform{e} : \timestamp^2 \times \svsorts \times \epsorts{e} \rarr \svsorts$ for each \ename $e$.
...
\item For each party or oracle \erule $r$ that \governs an \ename $e$:
	\begin{LPPI} 
	\item $\epconstr{r} : \timestamp^2 \times \svsorts \times \epsorts{e} \rarr \Bool$
	\end{LPPI}
\item For each deadline \erule $r$ that \governs an \ename $e$.
	 \begin{LPPI}
	\item $\deadfn{r} : \timestamp \times \svsorts \rarr \timestamp$
	\item $\psetter{r} : \timestamp \times \svsorts \rarr \epsorts{e}$ 	
	\end{LPPI}
\end{LPPI}
...
\begin{PPI}
\item $\assign{x}{f}$ for some $x \in \statevars$ of sort $S$\footnote{i.e. $\svsorts(x) = S$} and some function $f$ of type $\timestamp^2 \times \svsorts \times \epsorts{e} \rarr S$.
\item $\cond{f}{U_1}{U_2}$ for some function $f$ of type $\timestamp^2 \times \svsorts \times \epsorts{e} \rarr \Bool$ and finite sets of \estatements{e} $U_1, U_2$. 
\end{PPI}
...
We now officially modify the specification of \lsm to say that each \statetransform{e} is a \stransimpl (instead of just being a set of functions of types $\{ \timestamp^2 \times \svsorts \times \epsorts{e} \rarr \svsorts(x) \ | \ x \in \statevars \}$). 
...
\item A mapping $\svsorts^M$ from $\statevar$ to \DT.
\item For each $e \in \ename$, a type for its parameters $\epsorts{e}^M \in \DT^*$.
...
\end{PPI}
\item An $\cF$ function $\reguard{r}^M : \timestamp \times \svsorts \rarr \Bool$ for each \erule $r$.
\item For each party or oracle \erule $r$ that \governs an \ename $e$:
\begin{PPI} 
\item An $\cF$-function $\epconstr{r}^M : \timestamp^2 \times \svsorts \times \epsorts{e} \rarr \Bool$
\end{PPI}
\item For each deadline \erule $r$ that \governs an \ename $e$.
	 \begin{PPI}
	\item An $\cF$-function $\deadfn{r}^M : \timestamp \times \svsorts \rarr \timestamp$
	\item For each $r$-parameter $x_i$ of sort $S_i \in \epsorts{e}$, an $\cF$-function $(\psetterpart{r}{i})^M :  \timestamp \times \svsorts \rarr S_i$
	\end{PPI}
%\item For each \ename $e$, and each sort $S_i \in \svsorts$, a function $(\stranspart{e}{i})^M : \timestamp^2 \times \svsorts^M \times \epsorts{e}^M \rarr S_i$.
\end{LPPI}
...
\item Add a parameter $\tnext$ of sort \timestamp to every \ename \footnote{i.e. $\epsorts{e}(\tnext) = \timestamp$ for every \ename $e$.} 
...
Let $\DT$ be $\sortsof{\cF}$. The relation $>_{ts} {:} \timestamp \times \timestamp \rarr \Bool$ must be in $\cF$ (and so \timestamp must be in \DT). A $\timelesslsmclass{\cF}$ model $M$ is given by the following components. 
\begin{LPPI}
...
\item For each $e \in \ename$, types for its parameters $\epsorts{e} \in \DT^*$.
\item For each $e \in \ename$, a \stransimpl $\statetransform{e}$, which is a set of $\estatements{e}$ that satisfies the \uuc. 
...
\item For each party or oracle \erule $r$ that \governs an \ename $e$
    	\begin{PPI} 
	\item An $\cF$-function $\epconstr{r} : \svsorts \times \epsorts{e} \rarr \Bool$
	\end{PPI}
\item For each deadline \erule $r$ that \governs an \ename $e$.
	\begin{PPI}
	\item An $\cF$ function $\reguard{r} :  \svsorts \rarr \Bool$ for each \erule $r$.
	\item For each sort $S_i \in \epsorts{e}$, an $\cF$-function $\psetterpart{e}{i} :  \svsorts \rarr S_i$
	\end{PPI}
\end{LPPI}
...
\newcommand{\actorrule}[3]{#1 \text{ may } (#2 \text{ where } #3)}
\newcommand{\deadrule}[3]{#1(#2) \text{ provided } #3}
...
\newcommand{\sv}[1]{q_{#1}}
\newcommand{\ep}[2]{x_{#1}^{#2}}
\newcommand{\rp}[2]{x_{#1}^{#2}}

%\newcommand{\fterms}{\EM{\cF-text{terms}}\xspace}
\newcommand{\fterms}{\termz{fterms}{\cF-\text{terms}}}
\newcommand{\fterm}{\termz{fterms}{\cF-\text{term}}}

\newcommand{\sexecvars}{\termz{sexecvars}{sexec-variables}}

\warn{This section is not ready for reading yet}

$v$ ranges over \sexecvars. \\
The set of \firstuse{\fterms}{fterms} is defined by:
\begin{PPI}
\item \sexecvars are \fterms. 
\item If $t_1,\ldots,t_k$ are \fterms and $f$ is the function symbol for some $k$-ary function in $\cF$, then $f(t_1,\ldots,t_k)$ is a \fterm.
\end{PPI}
$\psi$ ranges over \fterms of sort \Bool.

\noindent Introduce symbols
\begin{PPI}
\item $\sv{i}$ to stand for the $i$-th \statevar. 
\item $\ep{i}{e}$ to stand for the $i$-th parameter of \ename $e$. 
\item $\rp{i}{r}$ to stand for the $i$-th parameter of \erule $r$. 
\end{PPI}

Throughout, $\Gamma$ is a full mapping from $\statevar$ to \fterms of the correct type. Initially it is $\initvals$.

$P$ is an \fterm of sort \Bool.
...
\[
\infer{
	\SV ;\ P \vdash \yields{ \shandlerof{s} }{\rv}
}
{	
	s \in \sname
&	\erule r \in \shandlerof{s}
&	\SV; P  \vdash \yields{r[\SV]}{\rv}
%&	\stacktwo
%		{\full{\SV'}{P'} \text{ consistent}}
%		{\SV' = \SV \cup \vx}
%		{P' = P \wedge \psi(\vx)}
%&	\SV';\ P'  \vdash \yields{r}{\rv}
}
\]
...
\[
\infer{
	\SV ;\ P \vdash \yields{ \actorrule{R}{e}{ \psi }} {\rv} 
}
{	\stackone
		{P' = P \wedge \psi[\SV,x^e \mapsto \vv]}
		{\vv \leftarrow fresh(x^e)}
& 	\full{\SV}{P'} \text{ consistent}		
&	\SV;\ P'  \vdash \yields{\ehandlerof{e}[\SV, x^e \mapsto \vv]}{\rv} 
}
\]
...
Suppose the only \statevars in an \lsm $M$ are \Bool or enums, and suppose that for every \erule $r$ and \ename $e$ none of $\reguard{r}$, $\deadfn{r}$, $\epconstr{r}$, $\statetransform{e}$, or $\psetter{r}$ depend on their \timestamp arguments. Then $M$ is equivalent for formal verification purposes to a kind of compressed\footnote{via the boolean \statevars} deterministic finite state machine (FSM). If there are no \statevars, then $M$ is equivalent for formal verification purposes to a normal FSM. Many important properties about FSMs are decidable. For example, if the specification of such a model $M$ is given entirely in terms of a \statespace invariant, then full formal verification can be checked exhaustively by well-known methods.
