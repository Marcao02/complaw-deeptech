...
In Section \ref{lasmstatic}, we gave as much detail as we could about our formal verification progress for \lasms. To go further, we need a complete language, which \lasm is not. In particular, we specified that certain components are mathematical {\it functions} or {\it relations}, rather than expressions that define such functions or relations. Those components are:
\begin{LPPI}
\item $\statetransform{e} : \timestamp^2 \times \statevartypes \times \eparamtypes{e} \rightarrow \statevartypes$ for each \ename $e$.
\item $\reguard{r} \subseteq \timestamp \times \statevartypes$ for each deadline \erule $r$.
\item $\params{r} : \timestamp^2 \times \statevartypes \rightarrow \eparamtypes{e}$ for each deadline \erule $r$ than governs an \ename $e$.
\item $\epconstr{r} \subseteq \timestamp^2 \times \statevartypes \times \eparamtypes{e}$ for each party or oracle \erule $r$ that governs  an \ename $e$.
\end{LPPI}
