...
\newcommand{\lasms}{\termz{LASM}{LASMs}}
\newcommand{\lasmclass}[1]{\textsf{\hyperref[lasmclass]{LASM}}(#1)}
...
Sections \ref{basics} and \ref{lasmdef} define Legal Abstract State Machines. Section \ref{lasmstatic} documents our progress on static analysis for \lasms.
...
An \lasm $M$ also has an ordered finite set of symbols \firstuse{\statevars}{statevars}, and an assignment \firstuse{\VDom}{statevartypes} of a datatype from $\DT$ to each. Since the \statevars are ordered, we can take \VDom to be an element of $\DT^*$. $M$ also includes an initial setting \initvals of its \statevars.
...
Despite the lack of concreteness, we saw that there is enough detail that execution can be defined precisely. We {\it could} stop there, but that would mean leaving out of the coming sections some useful details of formal verification routines that are very likely to be needed in any \lasm-compatible DSL. 

In this section, we reduce the {\it description} of the abstraction level significantly, while maintaining the flexibility of being able to define \lasm-compatible languages that range from finite state machines to Turing complete languages.\footnote{Technically, \extlink{https://en.wikipedia.org/wiki/Turing_degree}{beyond Turing complete languages}, but we don't know of any practical uses of such languages!}
Recall that that the set of basic datatypes or {\it sorts} \DT is a parameter to \lasm. After this section, we will have a notion of {\it class of \lasm} that depends only on \DT and a set \cF of functions on the sorts in \DT; that is, each function in such a set \cF is of type $S_1 \times \cdots \times S_k \rarr S_0$ for some $k \geq 0$ and some  $S_0,\ldots,S_k \in \DT$. Note that, for the sake of this report, we take individual elements of the sorts $\DT$ to be $0$-ary functions in $\cF$.
...
We now officially modify the specification of \lasm to say that each \statetransform{e} is \stransimpl (instead of just being a set of functions of types $\{ \timestamp^2 \times \VDom \times \eparamtypes{e} \rarr \VDom(x) \ | \ x \in \statevars \}$). 

\subsection{Final Specification of \lasm}
Since the start of Section \ref{lasmdef}, we've elaborated some details of the initial definition of \lasm, to bring it closer to being a target for formal verification. Here we give just the concise final specification, without explanation of the semantics.

\medskip

Let $\DT$ be a set of sorts (i.e. datatypes, i.e. sets). Let $\cF$ be a set of functions each of type $S_1 \times \cdots \times S_k \rarr S_0$ for some $k \geq 0$ and some $S_0,\ldots,S_k \in \DT$. Then $\cF$ determines a class of \lasms that we denote $\lasmclass{\cF}$, which are defined as follows. For notational simplicity, and to reduce redundancy, we assume that every sort in $\DT$ is in the type of some function in $\cF$, so that we may uniquely determine $\DT$ from $\cF$. Let \firstuse{$\sortsof{\cF}$}{sortsof} be that unique determination.
%
%Definition is on
%\pagebreak

\begin{defn}[\firstuse{\lasmclass{\cF}}{lasmclass}]
Let $\DT$ be $\sortsof{\cF}$. An $\lasmclass{\cF}$ model $M$ is given the following components. To reduce clutter, we omit the superscript $^M$ except at the component's introduction:
\begin{LPPI}
\item $\timeunit^M$ in $\{\text{days, hours, minutes, seconds, \ldots}\}$, which we write unsuperscripted in the remainder.
\item Finite sets $\actor^M, \ename^M, \sname^M$, and $\statevar^M$, which we write unsuperscripted in the remainder.
