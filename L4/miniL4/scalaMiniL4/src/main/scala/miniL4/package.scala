...
package object miniL4 {
  ...
  class BugInCodebaseException(msg:String) extends Exception(msg)

  def warn(test:Boolean, msg:String) : Unit = if(test) println("WARNING: " + msg) else ()
  def bugassert(test:Boolean, msg:String) : Unit = if(!test) throw new BugInCodebaseException(msg) else ()
